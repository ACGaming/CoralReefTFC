...
public class GeneratorReef implements IWorldGenerator
{
    ...
    private final WorldGenerator genReef;
    private final WorldGenerator genReefRock;
    ...
    private void reef(World world, Random rand, int chunkX, int chunkZ)
    {
        ...
        for (int x = 0; x < CHUNK_SIZE; x++)
        {
            for (int z = 0; z < CHUNK_SIZE; z++)
            {
                ...
                {
                    if (ArrayUtils.isEmpty(CoralReef.ConfigHandler.biomes) || ArrayUtils.contains(CoralReef.ConfigHandler.biomes, world.getBiome(pos).getRegistryName().getPath().toLowerCase()))
